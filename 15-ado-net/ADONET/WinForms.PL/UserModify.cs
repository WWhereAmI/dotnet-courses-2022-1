        ...
        private void btnModifyUser_Click(object sender, EventArgs e)
        {
            ...
            if (IsEnititesIsValid())
            {
                if (workMode == ModifyRecord.AddNew)
                {
                    ...
                    user.ID = personBL.AddUser(user);
                }
                else if (workMode == ModifyRecord.EditExisting)
                {
                    personBL.UpdateUser(user.ID, tbFirstNameUser.Text, tbLastNameUser.Text, tbBitrhDateUser.Value);
                }

                //Adding awards from UI
                foreach (var award in lbUserAwards.Items)
                {
                    if (award is Award newAward && !user.UserAwards.Contains(newAward))
                    {
                        personBL.AddUserAward(user, newAward);
                    }
                }

                foreach (var award in lbAvailableAwards.Items)
                {
                    if (award is Award awardToDelete && user.UserAwards.Contains(awardToDelete))
                    {
                        personBL.RemoveAward(awardToDelete);
                    }
                }
